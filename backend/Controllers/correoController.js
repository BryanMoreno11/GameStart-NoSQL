...
const pdfFonts = require('pdfmake/build/vfs_fonts'); // Importa fuentes para pdfMake
...
async function enviarCorreoPrueba(req, res) {
  let body = req.body;
  console.log('[enviarCorreoPrueba] Body recibido:', body);

  // Calcular el total, si es necesario (además se usa body.total_venta para la tabla final)
  ...
  const fechaFormateada = `${fechaVenta.getDate().toString().padStart(2, '0')}/${(fechaVenta.getMonth() + 1)
    .toString()
    .padStart(2, '0')}/${fechaVenta.getFullYear()}`;
    ...
    let pdfDefinition = {
      content: [
                ...
                stack: [
                  {
                    columns: [
                      ...
                      {
                        text: body.id_venta, // Número de factura
                        bold: true,
                        color: '#333333',
                        fontSize: 12,
                        alignment: 'right',
                  ...
                  },
                  {
                    columns: [
                      ...
                      {
                        text: fechaFormateada, // Fecha formateada
                        bold: true,
                        color: '#333333',
                        fontSize: 12,
                        alignment: 'right',
        ...
        {
          columns: [
            { text: 'GameStart', bold: true, color: '#333333', alignment: 'left' },
            { text: body.nombre, bold: true, color: '#333333', alignment: 'left' },
          ],
        },
        {
          columns: [
          ...
          table: {
            ...
            body: [
              ...
              ...body.productos.flatMap(item => {
                const esDigital = item.tipo && item.tipo.nombre === "Digital";
                ...
                let clavesRows = [];
                if (esDigital) {
                  console.log(`[enviarCorreoPrueba] Producto digital "${item.nombre}" claves disponibles:`, clavesDelProducto);
                  clavesRows = clavesDelProducto.slice(0, item.cantidad).map(clave => {
                    return [
                      {
                        text: clave,
                        margin: [10, 2, 0, 2],
                ...
                return [baseRow, ...clavesRows];
              })
            ]
          }
        },
        { text: '\n' },
        // Nueva tabla para mostrar Subtotal, IVA y Total de forma clara
        {
          layout: 'noBorders',
          table: {
            widths: ['*', 100],
            body: [
              [
                { text: 'SUBTOTAL:', bold: true, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5', },
                { text: `$${body.subtotal}`, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5', }
              ],
              [
                { text: 'IVA (15%):', bold: true, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5', },
                { text: `$${body.iva}`, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5' }
              ],
              [
                { text: 'TOTAL:', bold: true, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5', },
                { text: `$${body.total_venta}`, alignment: 'right', margin: [0, 5, 0, 5], fillColor: '#eaf2f5', }
              ]
            ]
          },
          margin: [0, 20, 0, 0]
  ...
  try {
    ...
    console.log('[enviarCorreoPrueba] PDF generado exitosamente.');
    ...
    config.sendMail(opciones, function(error, result) {
      if (error) {
        console.log('[enviarCorreoPrueba] Error al enviar correo:', error);
        res.status(500).json({
          ok: false,
          msg: error
        });
      } else {
        console.log('[enviarCorreoPrueba] Correo enviado:', result.response);
        res.json({
          ok: true,
          msg: result
        });
    ...
    console.log('[enviarCorreoPrueba] Error en el envío:', err);
